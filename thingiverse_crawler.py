...
def get_downloads(text):
    pattern = "(\d+\s)Downloads"
    matched = re.findall(pattern, text)
    return [int(val) for val in matched]
...
def crawl_things(N, output_dir, term=None, category=None, source=None, organize=False):
    ...
    page = 0
    previous_path = ''
    print("start by looking for ", baseurl)
...
#        print("looking at thus ",contents.text)
        print(parse_thing_ids)
        for thing_id in parse_thing_ids(contents.text):
            print("parsing ",thing_id)
        print("------------------------------------------")
        for thing_id in parse_thing_ids(contents.text):
            print("parsing ",thing_id)
            if thing_id in thing_ids:
                continue
            print("got this thing id: {}".format(thing_id))
            thing_ids.add(thing_id)
	    #print (thing_ids)
            license, thing_files, downloads, views = get_thing(thing_id)

            print ("downloads ",downloads[0])
	    records.append((thing_id, downloads[0], views[0]))
...
def get_thing(thing_id):
    ...
    print ("searching a sub page ",thing_id)
    downlo =  get_downloads(contents)
    views  =  get_views(contents)
    #print ("dwns ",downlo)
    license = parse_license(contents)
    return license, parse_file_ids(contents),downlo,views
